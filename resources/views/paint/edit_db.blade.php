...
<head>
    ...
    <title>Chỉnh sửa tranh - Admin</title>
    ...
    <div class="flex">
        ...
        <div class="main-content p-6 md:p-8">
            ...
            <div class="bg-white rounded-xl shadow-md overflow-hidden card-hover transition duration-300">
                <div class="p-6">
                    <form action="{{ route('painting_db.update', $painting->id_db) }}" method="POST" enctype="multipart/form-data" id="updateForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="painting_title" class="w-full px-4 py-3 form-input rounded-lg" 
                                       value="{{ old('painting_title', $painting->painting_title) }}" required>
                            </div>

                            <!-- Artist -->
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Họa sĩ</label>
                                <input type="text" name="artist_db" class="w-full px-4 py-3 form-input rounded-lg" 
                                       value="{{ old('artist_db', $painting->artist_db) }}" required>
                            </div>

                            <!-- Style -->
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Phong cách</label>
                                <input type="text" name="style_db" class="w-full px-4 py-3 form-input rounded-lg" 
                                       value="{{ old('style_db', $painting->style_db) }}" required>
                            </div>

                            <!-- Photographer -->
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Nhiếp ảnh gia</label>
                                <input type="text" name="photographer" class="w-full px-4 py-3 form-input rounded-lg" 
                                       value="{{ old('photographer', $painting->photographer) }}" required>
                            </div>

                            <!-- Similarity (disabled) -->
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Độ tương đồng</label>
                                <input type="text" class="w-full px-4 py-3 form-input rounded-lg bg-gray-100" 
                                       value="{{ old('similarity', $painting->similarity) }}" disabled>
                            ...
                            <div class="form-group">
                                ...
                                <div class="file-input-wrapper">
                                    ...
                                    <input type="file" id="img_url_db" name="img_url_db" class="file-input" accept="image/*">
                                </div>
                                @if($painting->img_url_db)
                                    <p class="mt-2 text-sm text-gray-600">Ảnh hiện tại:</p>
                                    <img src="{{ asset($painting->img_url_db) }}" alt="Current painting" class="current-image">
                                @endif
                            </div>
                            ...
                            <div class="form-group md:col-span-2">
                                ...
                                <textarea name="description" rows="4" class="w-full px-4 py-3 form-input rounded-lg" required>{{ old('description', $painting->description) }}</textarea>
                            </div>
                        </div>
